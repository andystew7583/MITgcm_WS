...
%%%
%%% expdir        Directory in which experiment is contained
%%%
%%% expname       Experiment name
%%%
%%% outdir        Directory into which to write products as a .mat file
%%%
%%% tmin          Time in years at which to start averaging
%%%
%%% tmax          Time in years at which to stop averaging  
%%%
%%% calc_psi_eddy Set true to compute eddy-induced transports
%%%
%%% deform_cavity Set true to deform coordinates in the cavity 
%%%
%%% use_PsiBT     Set true to use barotropic streamfunction as the coordinate system
%%%
%%% use_layers    Set true to use output from the Layers package to calculate isopycnal
%%%               fluxes. N.B. if this option is selected then the density variable must
%%%               be 'PD0' (surface-referenced potential density)
%%%
%%% gl_coord      Set true to use a coordinate system that aligns with the ice front
%%%
%%%
function calcOverturning (expdir,expname,outdir,tmin,tmax,calc_psi_eddy,deform_cavity,use_PsiBT,use_layers,gl_coord)
  ...
  %%% Construct total overturning (redundant if we're using LAYERS)
  ...
  save(fullfile(outdir,outfname),'-v7.3', ...
    'eta','ETA','dens_levs','times', ...
    'psi_tot','psi_mean','psi_mean_stand','psi_mean_fluc', ...
    'uvel_tavg','vvel_tavg','dens_tavg');
  
  if (calc_psi_eddy)
    save(fullfile(outdir,outfname),'psi_eddy', ...

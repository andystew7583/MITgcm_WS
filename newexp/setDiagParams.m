...
function [DIAG_PARM,DIAG_MATLAB_PARM,ndiags] = setDiagParams(use_shelfice,use_flux_diags,use_eddy_diags,use_layers,diag_freq_avg,diag_freq_inst)
  ...
  %%% Standard configuration for hires_seq_ experiments as well as WC_
  %%% experiments
  % diag_fields_avg = ...
  % { ...
  ...
  % ocean surface heat loss = SIqnet or oceQnet
  % ocean-to-ice heat loss  = SIqneti
  % open-ocean atmos loss   = SIqneto
  % ice-to-atmos heat loss  = SIatmQnt
  % ice-shelf heat loss     = SHIhtFlx
  % SIempmr = net freshwater flux applied to ocean surface after sea-ice effects
  % SIatmFW = net atmospheric freshwater flux into the ice/snow-covered column
  % atmosphere → open ocean:  (1 - SIarea) * EXFtaux, (1 - SIarea) * EXFtauy
  % atmosphere → sea ice:     SIfu, SIfv
  % atmosphere + sea ice → ocean:          oceTAUX/oceTAUY
  % ice shelf → ocean:        SHIUDrag, SHIVDrag
  diag_fields_avg = ...
  { ...
    'SIfu','SIfv','oceTAUX','oceTAUY','EXFtaux','EXFtauy',...
    'TFLUX','SFLUX',...
    'SIqnet','SIatmQnt','SIqneto','SIqneti',...
    'SIempmr','SIatmFW', ...
     ...
  };
